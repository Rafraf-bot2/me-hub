...
- [x] Secrets repo posés via `gh secret set` depuis `.env` : **`LB_COOKIE`** + **`TMDB_READ_TOKEN`** (2026-06-27). (Plus de `LB_USER`/`LB_PASS` — auto-login mort.)
- [x] **Run de test `workflow_dispatch` VERT** (run 28270166498, 2m13s) : auth OK, scrape complet, commit auto `aebfbf2` poussé. Galerie vérifiée **restée à 70 films / 1101 frames** (pas de régression à 33 → l'auth a tenu). NB : chaque run réécrit tout `cine.json` (generatedAt + tie-break `Math.random()` de `pickFrames` rebrassent l'ordre) → gros diff bénin à chaque sync hebdo.
