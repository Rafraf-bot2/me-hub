...
- [ ] **Partie B — Pont téléphone** : **GUIDE RÉDIGÉ → `tasks/sport-health-bridge.md`**. Lecteur HC = **plugin gratuit [TaskerHealthConnect](https://github.com/RafhaanShah/TaskerHealthConnect)**. Hôte : **Tasker** (~4€, étape 3A) OU **Automate LlamaLab GRATUIT** (étape 3B, capte bien la variable de retour du plugin ; ⚠️ MacroDroid non — il ignore les variables Tasker). → HTTP Request `POST /ingest/health` (header `x-ingest-token`, body `{date, steps, kcal_in, kcal_out, protein_g, carbs_g, fat_g, weight_kg}` — null tolérés). ⚠️ **snapshot complet du jour à chaque run** (l'upsert écrase par date). Test endpoint au curl d'abord (étape 0 du guide), puis Tasker, puis whitelist Doze. **= exécution côté auteur (sur le tel).**
